				...
				if (jQuery.inArray('existed', ruleArr) != -1 && fieldValue) {
					$http.get("/validateUser", {
						...
						if (+response.data.errorCode == 2) {
							var notification = (response.data.isNotValid) ? fieldLabel + ' is existed' : '';
				...
				if (jQuery.inArray('email', ruleArr) != -1 && fieldValue) {
					$http.get("/validateUser", {
						...
						if (+response.data.errorCode == 3) {
							var notification = (response.data.isNotValid) ? fieldLabel + ' is wrong format' : '';
							deferred.resolve(notification);
						}
					});
				...
				if (jQuery.inArray('alphanumberic', ruleArr) != -1 && fieldValue) {
					$http.get("/validateUser", {
						...
						if (+response.data.errorCode == 4) {
							var notification = (response.data.isNotValid) ? fieldLabel + ' is contain special character' : '';
							deferred.resolve(notification);
						}
					});
				}
